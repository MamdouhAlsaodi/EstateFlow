# EF-201 Replacement Implementation Plan — R2

## Plan metadata

- **Task:** EF-201-PLAN-R2
- **Planner lane:** `openai-codex/gpt-5.6-luna`, low thinking, read-only
- **Status:** PASS — corrected replacement plan only; G2/G3/G4 are not granted.
...
- **Replacement artifact:** `/home/server/projects/estateflow/docs/handoffs/EF-201-PLAN-R2/implementation-plan.md`
...
```bash
source ~/.nvm/nvm.sh && nvm use 24.14.1 && cd /home/server/projects/estateflow && ALLOW_DESTRUCTIVE_TESTS=1 pnpm db:test:guard
...
**Depends on:** T4 executor/review `PASS`, explicit `continue T5`, then `execute T5`. **Allowed path:** only `docs/handoffs/EF-201/T5-verification.md`. Re-run direct named unit/integration/HTTP commands above, including the corrected `source ~/.nvm/nvm.sh && nvm use 24.14.1 && cd /home/server/projects/estateflow && ALLOW_DESTRUCTIVE_TESTS=1 pnpm db:test:guard` invocation before each destructive integration sequence, plus the canonical regression commands, with the nvm precondition on every command, and independently confirm role cells, lifecycle, tenant isolation, version conflicts, search, image limits, error contract, no public exposure, no audit event, and no UI.
...
- `/home/server/projects/estateflow/docs/handoffs/EF-201-PLAN-R2/implementation-plan.md`

### Commands/actions

- Read-only inspection of the packet-declared R1 plan, independent review, `package.json`, and `scripts/assert-test-database.mjs`.
- Confirmed the review-required guard syntax from the guard implementation and package script.
...
**PASS** — the independent-review guard blocker is mechanically corrected in T3, T4, and the T5 verification instruction; no other plan scope or product decision changed. The next human decision is exact G2 approval; no executor may begin before it.
