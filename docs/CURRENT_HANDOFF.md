...
- EF-403 — Safe listing-to-content generation continuing Phase 4: closed allowlisted property projection (title/propertyType/addressText + identity only; `ownerReference`, internal ids, and timestamps structurally unreachable via the projection type and the SELECT itself), ten deterministic per-channel templates (version 1) where generation = f(propertyVersion, templateVersion, channel) with **no LLM calls anywhere**, missing facts rendered as visible `[PRICE]`/`[AREA]`/`[BEDROOMS]`/`[BATHROOMS]` placeholders — never invented, defaulted, or numeric-filled — a typed legal-claim denylist (normalized Arabic + English guarantee wording) rejecting rendered copy including tainted property values, full provenance stamp (source property id, property version, template id, template version) persisted atomically with the audited IDEA→DRAFT transition and enforced at the database (all-or-nothing CHECK, positive-version CHECKs, composite tenant FK to Property, and the EF-402 identity-immutability trigger extended in place to cover the stamp), revisions inheriting the stamp unchanged, guarded `POST /organizations/:id/content/generate` + `GET /organizations/:id/content/generation-templates` with Owner/Manager/Broker authority and tenant-safe 404s, OpenAPI document regeneration (closed-world typed client inventory unchanged, consistent with EF-402), Arabic-first generate-to-review flow on the content workspace (template list → generate from property → draft with highlighted placeholders → provenance line → review-queue link) plus the stamp on the content detail view, and four new `ef403-*` API test files proving determinism, placeholder correctness, denylist rejection, allowlist leakage prevention, authority, tenant isolation, and the full EF-402 lifecycle walk (DRAFT→REVIEW→APPROVED→SCHEDULED→PUBLISHED with hash lock) on `estateflow_test`: **PASS within packet boundary**. Evidence: `docs/handoffs/EF-403/implementation.md`.
- EF-404 — Publishing adapters recovery: deterministic `PublishingChannelPort` fake only, transactional at-most-once occurrences, replay/restart-safe worker delivery, cancel-before-delivery, exact payload snapshots with provenance, typed failure reasons, Arabic scheduled-delivery/results tabs, and guarded PostgreSQL plus composed worker-path proofs: **PASS within packet boundary**. Evidence: `docs/handoffs/EF-404/implementation.md`.
- EF-405 — Marketing analytics: read-only tenant-scoped campaign and organization rollups over approved expenses, budgets, Lead touches/first-last attribution, published ContentItems, derived CPL/CAC/ROI with `not enough data`, freshness-stamped Owner/Manager GET routes, strict OpenAPI/client contracts, Arabic responsive campaign analytics, reconciliation/authority/tenant tests: **PASS within packet boundary**. Evidence: `docs/handoffs/EF-405/implementation.md`.
- EF-502 — Viewing automation: deterministic 24h/1h reminder occurrences through the existing EF-302 scheduler tick, reschedule/cancel reset and void semantics, EF-305 in-app/template fallback delivery, post-completion outcome request, idempotent follow-up task, suggested-only Lead transition, Arabic viewing detail reminders, and worker exactly-once proof: **PASS within packet boundary**. Evidence: `docs/handoffs/EF-502/implementation.md`.
- EF-510 — Geo search: normalized property latitude/longitude with a PostGIS geography point and GiST index, tenant-scoped published-listing radius/polygon/bbox search with existing text/type filters, stable keyset cursors, server-side grid clusters, guarded HTTP routes, and Arabic-first minimal map/list UI: **PASS within packet boundary**. Evidence: `docs/handoffs/EF-510/implementation.md`.
- EF-601 — Media pipeline opening Phase 6: `StoragePort` abstraction with a deterministic in-memory fake adapter as the only activated implementation (S3-compatible adapter behind the same port documented and deliberately NOT activated without credentials), short-lived HMAC-signed upload intents bound to org/user/property/kind/content-type/size with direct-to-storage uploads (the API never proxies file bytes in business commands), confirm-time magic-byte MIME sniffing with declared-vs-actual matching, per-kind and per-intent size limits, polyglot marker scanning, intrinsic image-dimension sanity, and double-extension filename rejection — all as typed errors proven by a malicious payload suite, deterministic THUMB/PREVIEW variant derivation persisted at confirm, video metadata with a `PROCESSING` placeholder (`VIDEO_TRANSCODE_OUT_OF_SCOPE`), orphan TTL mark+sweep that never deletes confirmed/processing media (API-callable maintenance plus the third half of the single worker tick), `Property.coverMediaId` cover link, opaque `et1_…` storage keys with no server filesystem paths in any DTO, guarded Arabic-first property media workspace at `/ar/organizations/:organizationId/properties/:propertyId` (intent → direct upload → confirm → grid with variants, cover, delete), and unit+PostgreSQL integration proof including intent-binding, authority, tenant-isolation, and TTL-sweep tests on `estateflow_test`: **PASS within packet boundary**. Evidence: `docs/handoffs/EF-601/implementation.md`.
- EF-610 — Contracts: org-scoped versioned contract templates (EF-305 approval/immutable-version patterns) with strict snapshot-only placeholder allowlist, deterministic generation from (approved template version, immutable deal/property snapshot) with full provenance stamp, a zero-dependency in-repo deterministic PDF 1.4 writer with SHA-256 document hash recorded at generation and hash-verified regeneration (Arabic text as lossless `U+XXXX` escapes until EF-630 font embedding), database-trigger-enforced immutability of content/snapshot/provenance, strictly sequential signing over ordered deal parties with typed out-of-order rejection and DB-level order/signer/hash validation, FINALIZED state that fully freezes the row, append-only audit events for generate/sign/amend-request/finalize/void, Owner-only void with mandatory reason, guarded HTTP routes (excluded from the closed-world OpenAPI per EF-601 precedent), Arabic-first contracts workspace with the mandatory «توقيع تشغيلي مبسّط — ليس توقيعًا قانونيًا معتمدًا» banner on UI and documents, and unit + PostgreSQL integration proof on `estateflow_test`: **PASS within packet boundary**. Evidence: `docs/handoffs/EF-610/implementation.md`.
- EF-620 — Admin and moderation continuing Phase 6: platform-admin surface on the existing EF-121 `PLATFORM_ADMIN` role (no new roles) with a distinct `PlatformAdminGuard` plus application-layer re-checks, cross-tenant pending-broker queue reusing the EF-121 platform approval verbatim, platform-level broker suspend/reinstate with mandatory reasons, listing moderation state machine (`moderationStatus` default PENDING; approve keeps public, reject/takedown archive with mandatory reason, DB CHECK-enforced) with transactional append-only `ListingModerationEvent`, step-up password re-authentication bound to `(userId, accessSessionId)` with a 10-minute window and bounded deny limit (append-only `AdminStepUpEvent`), append-only DB-trigger-enforced `AdminAuditEvent` trail with bounded keyset-filtered search, cross-tenant failed automation-job review via parameterized raw SQL (typed failure reasons only, never execution keys or payloads), eleven guarded admin routes excluded from the closed-world OpenAPI per the EF-601/610 precedent, Arabic-first lean admin console (brokers, listings, audit search, failed jobs, step-up re-auth UI) with strict normalizers, and unit + PostgreSQL integration proof on `estateflow_test` including tamper-proof trigger tests and real-HTTP boundary verification: **PASS within packet boundary**. Evidence: `docs/handoffs/EF-620/implementation.md`.
- EF-630 — Arabic/English i18n + RTL/LTR closing Phase 6 (**COMPLETE after R2**): typed ar/en translation catalog under `apps/web/src/i18n/` with the plan's missing-key check enforced twice (compile time: `en: Record<keyof typeof ar, string>` fails `pnpm typecheck`/`pnpm build`; runtime: `src/test/i18n.test.ts`), a single locale/direction switch (middleware derives the locale from the URL segment into `x-estateflow-locale`; the async root layout is the only renderer of `<html lang dir>` via `directionFor()`; client `useT()`/server `getServerT()` share one mechanism; `app/en` stays deferred), locale-aware ar-default date/time/number/currency/percent/file-size formatters replacing every ad-hoc `Intl.DateTimeFormat("ar", …)` call site with bigint-safe minor-unit money that rejects fractional amounts, a hardcoded-Arabic-literal scan (`src/test/i18n-hardcoded.test.ts`) with a bounded, reason-documented, shrinking allowlist, sweeping of **all twelve** feature string surfaces plus all model-level label maps to catalog keys — R2 (2026-09-24) completed the final content/contracts/finance sweep (17 files) and emptied the hardcoded-literal scan allowlist to only the permanent synthetic-demo-data exemption — PDF Arabic verified as lossless `U+XXXX` escapes with the font-embedding limitation explicitly documented (binary font asset = forbidden new dependency), and all gates green (web 108/108 incl. the two new i18n suites; API integration 33 files on `estateflow_test` loopback:55435; build with middleware; drift PASS; the only integration failure is the documented pre-existing EF-502 worker time-bomb, outside packet paths and being fixed separately) (`docs/handoffs/EF-630/implementation.md`).

EF-120, EF-121, EF-201, EF-202, EF-203, EF-231, EF-232, EF-233, EF-234, EF-235, EF-301, EF-302, EF-303, EF-304, EF-305, EF-306, EF-401, EF-402, EF-403, EF-404, EF-405, EF-501, EF-502, EF-510, EF-601, EF-610, and EF-620 are closed within their documented boundaries. **Phase 3 (Automation & Reminders) is COMPLETE; Phase 4 (Content & Campaigns) is COMPLETE; Phase 5 is COMPLETE; Phase 6 (Media, Contracts, Admin & i18n) is COMPLETE — EF-630 closed by R2 with every feature string surface in the catalog and the hardcoded-literal allowlist emptied.**
...
**EF-630 is COMPLETE within its packet boundary (R1 + R2).** The i18n infrastructure is complete and enforced: typed ar/en catalog with the plan's missing-key check (typecheck + runtime), the single direction switch (middleware → `x-estateflow-locale` → root layout `<html lang dir>`), locale-aware ar-default formatters replacing all ad-hoc formatting, and PDF Arabic verified as lossless `U+XXXX` escapes with the font-embedding limitation documented (a binary font asset is a forbidden new dependency; shaping/bidi would also be required). All twelve feature string surfaces now live in the catalog (shell, leads, viewings, search, properties, admin, automation, campaigns, demo UI copy, and — via R2 — content, contracts, finance + all model label maps); the hardcoded-literal scan allowlist holds only the permanent synthetic-demo-data exemption, so regressions are impossible. Note: the EF-502 worker integration test has a documented pre-existing time-bomb (fails 21:00–24:00 UTC) that is being fixed separately. Phase 6 is fully closed; Phase 7 opens with **EF-701 — Security audit**. Evidence: `docs/handoffs/EF-630/implementation.md`.
