...
**EF-620 is CLOSED / PASS within its packet boundary.** Admin and moderation are complete: broker approval/suspension at platform level (reusing the EF-121 `PLATFORM_ADMIN` role and approval application), listing moderation with mandatory reasons and append-only audited transitions, step-up password re-authentication for sensitive privileged actions, bounded append-only audit search, and cross-tenant failed-jobs review — all behind a `PlatformAdminGuard` with Arabic-first admin console views. The next task is **EF-630 — Arabic/English** (complete translation inventory, RTL/LTR visual regression, locale-aware dates/numbers/currency, PDF Arabic font embedding, no hardcoded visible strings in feature code), closing Phase 6. Evidence: `docs/handoffs/EF-620/implementation.md`.
