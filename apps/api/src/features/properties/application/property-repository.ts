...
export const propertyFields = ["id", "organizationId", "title", "propertyType", "addressText", "ownerReference", "status", "version", "createdAt", "updatedAt"] as const;
export const listingFields = ["id", "organizationId", "propertyId", "status", "version", "createdAt", "updatedAt"] as const;
export const imageMetadataFields = ["id", "listingId", "mediaType", "byteSize", "position", "createdAt", "updatedAt"] as const;
...
export type PropertyRepositoryConflictCode = (typeof PropertyRepositoryConflictCode)[keyof typeof PropertyRepositoryConflictCode];
...
export interface PropertyRepository {
  findProperty(organizationId: string, propertyId: string): Promise<Property | null>;
  searchProperties?(organizationId: string, criteria: PropertySearchCriteria, cursor?: string, limit?: number): Promise<PropertySearchPage>;
  createProperty?(property: Property): Promise<Property>;
  updateProperty(input: { organizationId: string; propertyId: string; version: number; changes: Record<string, unknown> }): Promise<Property>;
  ...
  findActiveListingForProperty(organizationId: string, propertyId: string): Promise<Listing | null>;
  findListing?(organizationId: string, listingId: string): Promise<Listing | null>;
