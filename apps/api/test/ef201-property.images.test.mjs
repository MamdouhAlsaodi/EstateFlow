...
} from "../dist/features/properties/domain/image-metadata.js";
...
test("accepts JPEG, PNG, and WebP metadata within the 5 MiB limit", () => {
  for (const mediaType of ["JPEG", "PNG", "WEBP"]) {
    assert.deepEqual(
      ...
      { mediaType, byteSize: MAX_IMAGE_BYTES, position: 0 },
    );
  }
});
...
test("rejects unsupported types, oversized metadata, and more than ten records", () => {
  assert.throws(
    () =>
      validateImageMetadata({ mediaType: "GIF", byteSize: 10, position: 0 }),
    /media type/,
  );
  assert.throws(
    () =>
  ...
  assert.throws(
    () =>
      validateImageMetadata({ mediaType: "PNG", byteSize: 10, position: -1 }),
    /position/,
  );
...
test("image registration is metadata-only, scoped to management membership, and enforces the count before save", async () => {
  ...
  const repository = {
    async findListing(organizationId, listingId) {
      return organizationId === "org-1" && listingId === listing.id
        ? listing
        : null;
    },
    async countImages() {
  ...
  const image = await application.registerImageMetadata({
    actor: {
      verified: true,
      memberships: [{ organizationId: "org-1", role: "MANAGER", active: true }],
    },
    organizationId: "org-1",
