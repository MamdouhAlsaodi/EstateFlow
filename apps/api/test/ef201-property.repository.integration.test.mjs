...
import { PropertyRepositoryConflictError } from "../dist/features/properties/application/property-repository.js";
import {
...
test(
  "Prisma property repository persists scoped listings, image metadata, conflicts, and cleans up",
  {
  ...
  async () => {
    ...
    try {
      ...
      await prisma.organization.create({
        data: { id: organizationId, name: "EF-201 Synthetic Realty" },
      });
      ...
      assert.equal(
        (await repository.findProperty(organizationId, propertyId)).title,
        "Synthetic Villa",
      );
      ...
      await assert.rejects(
        ...
        (error) =>
          ...
          error.code === "ACTIVE_LISTING_CONFLICT",
      );
      ...
      assert.equal(
        (await prisma.property.findUnique({ where: { id: propertyId } })).title,
        "Updated Synthetic Villa",
      );
      await assert.rejects(
        () =>
      ...
      assert.deepEqual(
        await repository.listImageMetadata(organizationId, listingId),
        [image],
      );
      assert.deepEqual(
        await repository.listImageMetadata(randomUUID(), listingId),
        [],
      );

      const page = await repository.searchProperties(
        organizationId,
        { titleOrAddress: "Cairo" },
        undefined,
        1,
      );
