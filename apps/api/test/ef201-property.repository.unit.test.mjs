...
test("repository contract exposes a typed atomic listing conflict", async () => {
  const conflict = new PropertyRepositoryConflictError(
    "ACTIVE_LISTING_CONFLICT",
  );
...
test("repository contract exposes only the approved persisted fields", () => {
  ...
  assert.deepEqual(listingFields, [
    "id",
    "organizationId",
    "propertyId",
    "status",
  ...
  assert.deepEqual(imageMetadataFields, [
    "id",
    "listingId",
    "mediaType",
    "byteSize",
...
test("Prisma repository maps organization-scoped property search and opaque cursor", async () => {
  ...
  const result = await new PrismaPropertyRepository(prisma).searchProperties(
