...
} from "../dist/features/properties/application/property-application.js";
...
test("application update uses the current version and returns the next version", async () => {
  const repository = {
    async findProperty(organizationId, id) {
      return organizationId === property.organizationId && id === property.id
        ? property
        : null;
    },
    async updateProperty(input) {
      return { ...property, ...input.changes, version: input.version + 1 };
    },
  };
...
test("management commands require a verified actor with active organization membership", async () => {
  ...
  await assert.rejects(
    () =>
      application.createProperty({
        actor: { verified: false, memberships: [] },
        organizationId: property.organizationId,
        property: {},
      }),
    PropertyAccessDeniedError,
...
test("broker reads published listings only and client has no access", async () => {
  ...
  const broker = {
    verified: true,
    memberships: [
      { organizationId: property.organizationId, role: "BROKER", active: true },
    ],
  };
  assert.equal(
    await application.getListing({
      actor: broker,
      organizationId: property.organizationId,
...
test("broker property reads require a published active listing", async () => {
  const repository = {
    async findProperty() {
      return { ...property, activeListing: { status: ListingStatus.DRAFT } };
    },
  };
  const application = new PropertyApplication(repository);
  await assert.rejects(
...
test("image metadata reads are organization-scoped and broker-visible only when published", async () => {
  const published = {
    id: "listing-1",
    organizationId: property.organizationId,
    propertyId: property.id,
  ...
  const calls = [];
  const repository = {
    async findListing(organizationId, listingId) {
      calls.push(["findListing", organizationId, listingId]);
      return organizationId === property.organizationId &&
        listingId === published.id
        ? published
        : null;
    },
    async listImageMetadata(organizationId, listingId) {
      calls.push(["listImageMetadata", organizationId, listingId]);
      return images;
    },
  };
  const application = new PropertyApplication(repository);
  ...
  assert.deepEqual(
    await application.listImageMetadata({
      actor: broker,
      organizationId: property.organizationId,
      listingId: published.id,
    }),
  ...
  assert.deepEqual(calls, [
    ["findListing", property.organizationId, published.id],
    ["listImageMetadata", property.organizationId, published.id],
  ]);
  await assert.rejects(
    () =>
      application.listImageMetadata({
        actor: {
          verified: true,
          memberships: [
            {
...
test("inactive membership and platform admin do not gain implicit tenant access", async () => {
  ...
  await assert.rejects(
    () =>
      application.getProperty({
        actor: { verified: true, memberships: [], platformAdmin: true },
        ...request,
      }),
    PropertyNotFoundError,
  );
